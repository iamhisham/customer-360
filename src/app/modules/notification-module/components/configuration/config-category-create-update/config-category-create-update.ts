...
export class ConfigCategoryCreateUpdateComponent implements OnInit {
  ...
  constructor(private alertController: AlertController, private router: Router,
    public notifiService: NotificationService, private toastController: ToastController,
      ...
      if (this.isEditView) {
        this.notifiService.updateCategoryById(this.catagoryId, this.category).subscribe({
          next: (e: any) => {
            ...
            this.router.navigate(['/notification-center/category']);
          },
          error: (err: any) => {
            this.notifiService.hideLoader();
            err = err.error?.error || err.error || err;
  ...
  //onCancel popup
  async onCancel() {
    this.notifiService.showCancelConfirmation(['/notification-center/category']);
  }
