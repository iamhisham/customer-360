...
import { DynamicTagsModelComponent } from '../../models/dynamic-tags-model/dynamic-tags-model.component';
...
@Component({
  selector: 'app-notification-ui-create',
  templateUrl: './notification-ui-create.component.html',
  styleUrls: ['./notification-ui-create.component.scss'],
  animations: [
    trigger('fadeIn', [
...
export class NotificationUiCreateComponent implements OnInit {

  landingPageURL: string = '/notification-center/notification';
  ...
  constructor(private alertController: AlertController, public notifiService: NotificationService,
    private router: Router, private actRouter: ActivatedRoute, private modalController: ModalController,
    public commonService: CommonService) {
    this.isAbTesting = this.router.url.startsWith('/notification-center/ab-testing');
    if (this.isAbTesting) {
      this.landingPageURL = '/notification-center/ab-testing';
  ...
  getNotificationById() {
    this.notifiService.getNotificationById(this.notification_id, this.isAbTesting).subscribe(async (notification: any) => {
      ...
      if (this.notification.isBulkNotification) {
        ...
        console.log(this.notification.customerId);
        if (this.notification.customerId && this.notification.customerId !== 0) await this.getActivateChannelOfSingleUser(this.notification.customerId, false);
        console.log(this.notification.customerId);
      ...
      if (this.notification.scheduledAt) {
        ...
        this.scheduleTime = new Date(this.notification.scheduledAt); //todo :need to validate
      }
      if (this.isAbTesting) {
        this.isDirectContentMessage = false;
  ...
  isValidNotification(notification: any) {
    try {
      if (notification.isBulkNotification == true) {
        ...
        if (!notification.customerId) throw 'Please enter Customer Id';
      }
  ...
  async modalPresent(content: any) {
    try {
  ...
  async getActivateChannelOfSingleUser(id: number, showLoader: boolean = false) {
    ...
    try {
      if(id !=  undefined) this.singleUserCount = await this.notifiService.getActivateChannelOfSingleUser(id).toPromise();
