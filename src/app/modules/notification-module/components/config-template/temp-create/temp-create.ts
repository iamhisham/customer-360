...
import { TemplatePageComponent } from 'src/app/modules/notification-module/components/template-page/template-page.component';
...
export class TempCreate implements OnInit {
  ...
  constructor(private modalController: ModalController, public notifiService: NotificationService,
  ...
  async getAllCategory() {
    try {
      ...
      this.notifiService.toster.error(e.error?.error?.message || e.message || e || 'Delete Failed');
    } finally {
      this.notifiService.hideLoader();
    }
  ...
  getSelectedChannelList() {
    return this.channelList.filter((channel: any) => this.template.channels.indexOf(channel.name) != -1);
  }
    ...
    if (this.isEditView) {
      ...
      this.notifiService.updateTemplateById(this.template_id, template).subscribe({
        next: async (e: any) => {
          ...
          this.router.navigate(['/notification-center/template']);
        },
        error: async (err: any) => {
          this.notifiService.hideLoader();
          err = err.error?.error || err.error || err;
            ...
            try {
              if (selectedFile.type == "application/zip" || selectedFile.type == "application/x-zip-compressed") {
                ...
                reader.onload = async (e: any) => {
                  ...
                  const zip = new JSZip();
                  const zipFileData: any = await zip.loadAsync(arrayBuffer).catch((err: any) => {
                    this.notifiService.hideLoader();
